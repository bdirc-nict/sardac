...
var slice_slice = function(length) {
  ...
  return (function read() {
    return that._source.read().then(function(result) {
      ...
      // When done, it is possible the request wasn?t fully fullfilled!
...
var my_decode = function( arr ) {
    var unicodeArray = Encoding.convert(arr, {
      to: 'UNICODE',
      from: 'AUTO'
    });
    var ret = Encoding.codeToString(unicodeArray);
    // console.log(ret);
    return ret;
}
...
var dbf_read = function() {
  ...
  return that._source.slice(that._recordLength).then(function(value) {
    return value && (value[0] !== 0x1a) ? {done: false, value: that._fields.reduce(function(p, f) {
      p[f.name] = types[f.type](/*that._decode*/ my_decode(value.subarray(i, i += f.length)));
      return p;
    }, {})} : {done: true, value: undefined};
  });
};
...
function Dbf(source, decoder, head, body) {
  this._source = source;
  this._decode = null; // decoder.decode.bind(decoder);
  this._recordLength = head.getUint16(10, true);
  this._fields = [];
  for (var n = 0; body.getUint8(n) !== 0x0d; n += 32) {
    for (var j = 0; j < 11; ++j) if (body.getUint8(n + j) === 0) break;
    this._fields.push({
      name: /*this._decode*/ my_decode(new Uint8Array(body.buffer, body.byteOffset + n, j)),
...
var readMultiPoint = function(record) {
  ...
  if (this.typeVal == 18) {
      var zMin = record.getFloat64(i, true); 
      i += 8;
      var zMax = record.getFloat64(i, true); 
      i += 8;
      for (j = 0; j < n; ++j, i += 8) {
          var z = record.getFloat64(i, true);
          coordinates[j].push( z );
      }
  }

  return {type: "MultiPoint", coordinates: coordinates, typeVal: this.typeVal};
};
...
var readPoint = function(record) {
  if (this.typeVal == 1)
      return {type: "Point", coordinates: [record.getFloat64(4, true), record.getFloat64(12, true)], typeVal: this.typeVal};
  else
      return {type: "Point", coordinates: [record.getFloat64(4, true), record.getFloat64(12, true), record.getFloat64(20, true)], typeVal: this.typeVal};
};
...
var readPolygon = function(record) {
  ...
  return polygons.length === 1
      ? {type: "Polygon", coordinates: polygons[0], typeVal: this.typeVal}
      : {type: "MultiPolygon", coordinates: polygons, typeVal: this.typeVal};
};
...
var readPolyLine = function(record) {
  ...
  return n === 1
      ? {type: "LineString", coordinates: points, typeVal: this.typeVal}
      : {type: "MultiLineString", coordinates: parts.map(function(i, j) { return points.slice(i, parts[j + 1]); }), typeVal: this.typeVal};
};
...
function open(shp$$1, dbf$$1, options) {
  ...
  return Promise.all([shp$$1, dbf$$1]).then(function(sources) {
    var shp$$1 = sources[0], dbf$$1 = sources[1], encoding = "shift-jis";
...
function openDbf(source, options) {
  ...
  if (options && options.encoding != null) encoding = options.encoding;
  encoding = null; /*new TextDecoder(encoding);*/
  if (typeof source === "string") {
