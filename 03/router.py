...
#==================================================================================
#https://github.com/F6F/SimpleOsmRouterを引用
    ...
    def startElement(self, name, attrs):
        ...
        if name == "way":
            ...
            try:
                ...
                print("there isn't node " + attrs["ref"])
...
class connection():
    ...
    #added
    def getArea(self):
        if (self.node1.lat < self.node2.lat):
            lowerLatitude = self.node1.lat
            higherLatitude = self.node2.lat
        else:
            lowerLatitude = self.node2.lat
            higherLatitude = self.node1.lat

        if (self.node1.lon < self.node2.lon):
            lowerLongtitude = self.node1.lon
            higherLongtitude = self.node2.lon
        else:
            lowerLongtitude = self.node2.lon
            higherLongtitude = self.node1.lon

        return [(lowerLatitude, lowerLongtitude), (higherLatitude, higherLongtitude)]
...
#
#    db = routingDb()
#    print(readDb(open(filepath, "r", encoding="utf-8"), db))
#    db.cleanUp()
#    startRouting(db)